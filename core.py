...
_FALLBACK_PHRASES = {
    ...
    "chat_emotions":[":)","^^","GG","gg","¯\\_(ツ)_/¯","(╯°□°)╯︵ ┻━┻",">_<","xD"],
...
def calculate_price(seat_type, tariff_key, duration, promo_disc):
    if seat_type not in TARIFFS or tariff_key not in TARIFFS[seat_type]:
        return 0.0
    tf=TARIFFS[seat_type][tariff_key]
    base=tf["fixed"] if "fixed" in tf else max(0, duration)*tf["price_per_min"]
    promo_disc = clamp(promo_disc, 0.0, 1.0)
    return max(0.0, base*(1-promo_disc))

def get_valid_tariffs(seat_type, t, close, res_time=None):
    if seat_type not in TARIFFS:
        return ["min"]
    limit=close
...
def _seat_inc(seats, sid):
    ...
    if idx is None: return tuple(out)
...
def tick_entrance(st):
    ...
    ready = []
    still_waiting = []
    for es in st.entrance:
        if es.timer <= 0:
            ready.append(es.client)
        else:
            still_waiting.append(es._replace(timer=es.timer - 1))
    new_queue = list(st.queue) + ready
    return st._replace(entrance=tuple(still_waiting), queue=tuple(new_queue))
...
def tick_arrivals(st):
    ...
    entrance_list=list(st.entrance)
    ...
    # Клиенты перестают приходить за час до закрытия
    ...
    if timer<=0 and not too_late:
        ...
        if is_group:
            ...
            if cl.reservation:
                ...
                entrance_list.append(EntranceSlot(cl, 2))
                logs.append(Log(t,f"→ {cl.name} входит в клуб ({cl.pref})","arrival"))
                last=f"{cl.name} пришёл"
            nc+=1
    ...
    # Забронированные клиенты приходят напрямую в очередь (приоритет)
    ...
    return st._replace(
        queue=tuple(queue), reservations=tuple(remaining), lounge=tuple(lounge),
        entrance=tuple(entrance_list), lost_clients=lost,
        next_client=nc, next_group=ng, arrival_timer=timer,
        logs=tuple(logs), rng=rng, last_action=last,
    )
    ...
    if admin.status=="idle":
        ...
        if li is not None:
            ...
            if head.group is None:
                queue.pop(0); admin=AdminState("processing",1,head,None)
            else:
                members=[c for c in queue if c.group==head.group]
                queue=[c for c in queue if c.group!=head.group]
                proc,rng=rng_int(rng,2,3)
                admin=AdminState("processing",proc,head,members)

    elif admin.status in ("processing","seating_lounge"):
        if admin.timer>1:
            admin=admin._replace(timer=admin.timer-1)
        else:
            ...
            if members:
                ...
                if chosen is not None:
                    if len(chosen)==1 and chosen[0].type=="console":
                        ...
                        for m in members_to_seat:
                            seat_now=next(s for s in seats if s.id==sid_)
                            ses,rng=_start_ses(sid,m,seat_now,t,promos,rng)
                            sessions.append(ses); sid+=1; seats=_seat_inc(seats,sid_)
                ...
                if seat is not None:
                    ses,rng=_start_ses(sid,cl,seat,t,promos,rng)
...
def tick_behavior(st):
    t=st.time; rng=st.rng; promos=active_promotions(st); cur_disc=best_discount(promos)
    logs=list(st.logs); chats=list(st.chats); seats=st.seats
    orders=list(st.order_queue); next_o=st.next_order; rev=st.revenue; served=st.served
    kept=[]; ph=st.phrases_data; last=st.last_action; fs=st.food_stock
    bathroom=list(st.bathroom_clients)
    too_close = t >= CONFIG["close"] - 60   # последний час — не принимать заказы/продления
    ...
    for ses in st.sessions:
        food_cd=max(0,ses.food_cooldown-1)
        trig_cd=max(0,ses.trigger_cooldown-1)
        chat_tm=max(0,ses.chat_timer-1)
        ...
        if ses.seat_type=="pc" and chat_tm==0:
            ...
            if talk:
                if chat_st=="idle" and ph["chat_questions"]:
                    msg,rng=rng_pick(rng,ph["chat_questions"])
                    chats.append(ChatMsg(t,ses.client_name,msg,ses.seat_id,"pc_chat"))
                    chat_st="asked"
                elif chat_st=="asked" and ph["chat_answers"]:
                    msg,rng=rng_pick(rng,ph["chat_answers"])
                    chats.append(ChatMsg(t,ses.client_name,msg,ses.seat_id,"pc_chat"))
                    chat_st="answered"
                elif ph["chat_reactions"]:
                    msg,rng=rng_pick(rng,ph["chat_reactions"])
                    chats.append(ChatMsg(t,ses.client_name,msg,ses.seat_id,"pc_chat"))
                    chat_st="idle"
            else:
        ...
        if live_tm==0:
            ...
            if say:
                cat,rng=rng_pick(rng,("live_questions","live_answers","live_reactions"))
                pool=ph[cat]
                if pool:
        ...
        if trig_cd==0 and not too_close and not any(bc.session_id==ses.id for bc in new_bath):
            ...
            if fire:
                ...
                if want_food and pending is None and food_cd==0:
                    ...
                    if available_menu:
                        food,rng=rng_pick(rng,available_menu)
                        stock_items=dict(fs.items)
                        tpl,rng=rng_pick(rng,FOOD_TRIGGERS)
                        msg=tpl.format(food=food.name)
                        chats.append(ChatMsg(t,ses.client_name,msg,ses.seat_id,"trigger_food"))
                        orders.append(Order(next_o,ses.client_id,ses.id,food,"pending",None))
                        stock_items[food.name]=max(0, stock_items.get(food.name,0)-1)
                        fs=fs._replace(items=stock_items)
...
def tick_staff(st):
    t=st.time; rng=st.rng; workers=list(st.hall_workers)
    orders=list(st.order_queue); food_rev=st.food_revenue
    sessions=list(st.sessions); logs=list(st.logs); last=st.last_action
    ...
    for i,w in enumerate(workers):
        if w.status=="idle":
            ...
            if ntm<=0:
                ...
                target_seat=None
                if oi is not None:
                    o=orders.pop(oi); food_rev+=o.food.price
                    si=next((k for k,s in enumerate(sessions) if s.id==o.session_id),None)
                    if si is not None:
        ...
        elif w.status=="serving":
            # Работник общается с клиентом после доставки, затем возвращается в бар
            ntm=w.timer-1
            if ntm<=0:
                workers[i]=w._replace(status="idle",timer=0,target_seat_id=None)
            else:
                workers[i]=w._replace(timer=ntm)
...
def _close_all_sessions(state):
    ...
    for ses in state.sessions:
        tf = TARIFFS[ses.seat_type][ses.tariff]
        if tf.get("duration") is None:
            price = max(0.0, (t - ses.start) * tf["price_per_min"] * (1 - ses.promo_discount))
        else:
...
def _do_close(state, t):
    ...
    return state._replace(
        ...
        logs=state.logs+(close_log,),
        last_action=f"День {state.day} завершён. Прибыль: {fmt_money(profit)}",
    )
...
def tick(state):
    ...
    # Страховка: если время значительно вышло за close, принудительно закрываем
    if t > CONFIG["close"] + 10:
        st = _close_all_sessions(st)
        return _do_close(st, t+1)
...
def apply_command(state, action):
    ...
    if not isinstance(act, str) or act not in VALID_ACTIONS:
        return state
